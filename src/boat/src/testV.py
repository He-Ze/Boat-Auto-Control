...
class mpc_stanly_com(object):
	...
	def get_gps(self, msg):
		...
		if not had_arrive_the_dis:
			...
			if self.get_dis([boat[0], boat[1]], des[goal_num][:2]) < 15:
				goal_num += 1
...
def plot_ponit():
	global target_pointx, target_pointy, boat, target_num, goal_num,x1,x2,x3,x4,y1,y2,y3,y4
	n = 0
	Ex2=0
	x=[]
	y=[]
	...
	while True:
		...
		plt.ylabel(boat[1])
		plt.title(goal_num)
		plt.plot([x1,x2,x3,x4],[y1,y2,y3,y4],'go')
...
if __name__ == '__main__':
	...
	points_angle = []
	destination = [0,0]
	x1,y1 = gps_to_mkt1(113.61221177,22.37496951)
	x2,y2 = gps_to_mkt1(113.61236051,22.37549078)
	x3,y3 = gps_to_mkt1(113.61278028,22.37548521)
	x4,y4 = gps_to_mkt1(113.61221177,22.37496951)
	des = [[x1,y1,0],[x2,y2,0],[x3,y3,0],[x4,y4,0]]
